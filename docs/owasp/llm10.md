...
disproportionate work. That's the vulnerability. Nothing is missing from the measurement. Recording that as merely
"inconclusive" quietly under-reported the apps that consume the most.
...
!!! note "The budget is a wall-clock deadline, not a socket timeout"
    ...
    left so a target that goes quiet mid-answer cannot claim a second full budget. The buffered body
...
`output_tokens` result property), the concrete denial-of-wallet figure for that probe, and the run records a **`denial_of_wallet`** summary (total, peak and mean output tokens across every probe that
...
total token spend over time. A black-box `app:<url>` endpoint reports no usage, so it contributes
