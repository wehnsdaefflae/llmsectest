...
| `--app-timeout <seconds>` | With `--target app:<url>`: the **per-request time budget** for a single call to the app (default 120 s). A request that exceeds it is recorded as an **inconclusive** probe — not a finding (a timeout is not proof of a vulnerability) and not a silent clean, but a warning in the summary — so one slow or runaway endpoint cannot hang the whole scan and lose every other result. Raise it for a legitimately slow app; lower it to keep a scan under a fixed wall-clock budget (e.g. in CI). |
